...
int runRPBWT(string input, string output, int checkpoint) {
	ios_base::sync_with_stdio(0); cin.tie(0);

	ifstream in(input);
	ofstream out(output + ".rpbwt", ios::binary), meta(output + ".meta");
	...
	int site = 0;
	vector<int> pre(M_r), div(M_r); // prefix and divergence array
	iota(pre.begin(), pre.end(), 0);
	vector<int> a(M_r), b(M_r), d(M_r), e(M_r);
	char s[2 * M_r + 5000]; // assumes fixed fields take up less than 5000 characters

	in.seekg(-1, ios::end);
	if (in.peek() == '\n') in.seekg(-1, ios::cur); // the process won't work if it starts on a newline character
	in.seekg(-2 * M_r, ios::cur);
	while (in.tellg() >= 0) {
		// process for reading file backwards
		while (in.peek() != '\n') in.seekg(-1, ios::cur);
		streampos pos = in.tellg();
		in.seekg(1, ios::cur);
		in.getline(s, 2 * M_r + 5000);
